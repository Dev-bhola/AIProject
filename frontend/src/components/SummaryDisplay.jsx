    ...
    <div className="w-full max-w-4xl mx-auto mt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-slate-800/60 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6 shadow-xl">
        <header className="mb-6 border-b border-slate-700/50 pb-4">
          <h3 className="text-sm font-semibold text-blue-400 tracking-wider uppercase flex items-center gap-2 mb-2">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            Document Summary
          </h3>
          <h2 className="text-xl font-bold text-slate-100">{documentName || result.doc_id}</h2>
          {result.truncated && (
             <div className="mt-2 inline-flex items-center px-2 py-1 bg-yellow-900/30 text-yellow-500 text-xs rounded border border-yellow-700/30">
        ...
        <div className="space-y-8">
          <section>
            <h4 className="text-sm font-medium text-slate-400 uppercase tracking-wider mb-3">Overall Summary</h4>
          ...
          {result.summary_points && result.summary_points.length > 0 && (
            <section>
              <h4 className="text-sm font-medium text-slate-400 uppercase tracking-wider mb-4">Key Points</h4>
              <ul className="space-y-5">
                {result.summary_points.map((pt, idx) => (
                  <li key={idx} className="bg-slate-900/40 p-4 rounded-xl border border-slate-700/30">
                    <p className="text-slate-200 leading-relaxed text-base mb-2">
                      <strong className="text-slate-400 font-medium mr-2">{idx + 1}.</strong> 
                      {pt.point}
                    </p>
                    <div className="flex">
